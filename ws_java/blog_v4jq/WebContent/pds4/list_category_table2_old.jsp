...
<body>
    ...
    <DIV class='content'>
      ...
      <DIV class='aside_menu'>
        ...
        <ASIDE style='float: right;'>
          ...
          <A href='./list_category_table2.jsp?categoryno=<%=categoryno%>&col=<%=col%>&word=<%=word%>'>목록형</A> <span class='menu_divide'> |</span> <A
            href='./list.jsp?categoryno=<%=categoryno%>&col=<%=col%>&word=<%=word%>'>앨범형</A> <span class='menu_divide'> |</span> <A href='./list_read.jsp'>펼쳐보기</A>
        </ASIDE>
  ...
  --%>
      ...
      <TABLE class='table_basic'>
        ...
        <tbody>
          ...
          <TR style = 'height: 80px;'>
            ...
            <TD class = 'td_left'>
            <A href='./read.jsp?categoryno=<%=categoryno %>&pdsno=<%=pds4VO.getPdsno()%>'><%=Tool.convertChar(Tool.textLength(pds4VO.getTitle(), 50))%></A>
      ...
      %>
              <A href='./update_form.jsp?categoryno=<%=categoryno %>&pdsno=<%=pds4VO.getPdsno()%>'><IMG src='./images/update.png' title='수정'></A> <A
                href='./delete_form.jsp?categoryno=<%=categoryno %>&pdsno=<%=pds4VO.getPdsno()%>'><IMG src='./images/delete.png' title='삭제'></A>
            </TD>
      ...
      <DIV class='bottom_menu'>
        <button type='button' onclick="location.href='./create_form.jsp?categoryno=<%=categoryno%>'">등록</button>
